...
export function BanDialog({ open, onOpenChange, playerId, playerName, onBanConfirmed }: BanDialogProps) {
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              You are about to ban <span className="font-semibold">{playerName}</span>. This action will restrict their
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-3">
              ...
              <RadioGroup value={duration} onValueChange={(value) => setDuration(value as BanDuration)}>
                <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                  <RadioGroupItem value="1day" id="1day" />
                  <Label htmlFor="1day" className="cursor-pointer font-normal flex-1">
                    <p className="font-semibold">1 Day</p>
                    <p className="text-xs text-muted-foreground">Temporary ban for minor violations</p>
                  </Label>
                </div>
                <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                  <RadioGroupItem value="7days" id="7days" />
                  <Label htmlFor="7days" className="cursor-pointer font-normal flex-1">
                    <p className="font-semibold">7 Days</p>
                    <p className="text-xs text-muted-foreground">Standard ban for serious violations</p>
                  </Label>
                </div>
                <div className="flex items-center space-x-2 p-3 border border-destructive/30 rounded-lg hover:bg-destructive/5 transition-colors">
                  ...
                  <Label htmlFor="permanent" className="cursor-pointer font-normal flex-1">
                    <p className="font-semibold text-destructive">Permanent Ban</p>
                    <p className="text-xs text-muted-foreground">Irreversible ban for severe violations (cheating, hacking)</p>
                  </Label>
                </div>
              </RadioGroup>
            </div>
            ...
            {duration === "permanent" && (
              <div className="p-3 bg-destructive/10 border border-destructive/30 rounded-lg flex items-start gap-2">
                ...
                <div className="text-sm">
                  <p className="font-semibold text-destructive">Warning: Permanent Ban</p>
                  <p className="text-xs text-muted-foreground mt-1">
                    This action cannot be undone. The player will lose access forever. Use only for confirmed cheaters.
                  </p>
                </div>
              </div>
            )}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-3">
              <p>
                You are about to <span className="font-bold text-destructive">permanently ban</span>{" "}
